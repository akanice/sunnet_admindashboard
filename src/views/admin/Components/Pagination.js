// Version: 1.2.0
// Mô tả: Component phân trang tuỳ biến cho phép hiển thị tối đa 3 trang đầu và 3 trang cuối.
//        Các trang ở giữa sẽ được rút gọn bằng ký hiệu '...'. Khi đang ở trang giữa (không thuộc
//        3 trang đầu/cuối), component sẽ hiển thị trang liền trước, trang hiện tại và trang liền sau
//        theo dạng ví dụ: ...5,6,7...
...
const Pagination = ({ totalPages, currentPage, onPageChange, length, totalItems }) => {
  ...
  // Hàm sinh danh sách item trang cần hiển thị theo quy tắc rút gọn
  // - Mục tiêu: Hiển thị tối đa 3 trang đầu và 3 trang cuối
  // - Ở giữa: nếu currentPage không thuộc 3 trang đầu/cuối, hiển thị current-1, current, current+1
  // - Thêm '...' để rút gọn khi bị ngắt quãng giữa các nhóm trang
  const getPageItems = () => {
    ...
    // Nếu đang ở khối cuối, đảm bảo không bị trùng và bỏ bớt ... thừa
    if (inEndBlock) {
      // Nếu đã có 'ellipsis-right' nhưng sát với endPages, giữ nguyên; nếu không cần, vẫn an toàn hiển thị
    }
